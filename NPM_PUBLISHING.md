...
بسته `readme-press` در رجیستری عمومی npm ثبت شده و نسخه پایدار `0.1.3` با تگ `latest` در دسترسه. ارتباط Trusted Publishing هم برای مرحله‌بندی بسته از GitHub Actions تنظیم و در انتشارهای قبلی آزمایش شده.

نسخه بعدی `0.2.0` است. این نسخه خروجی اختیاری مناسب چاپ رو اضافه می‌کنه و همون مسیر امن و آزمایش‌شده برای انتشار پایدار رو طی می‌کنه: PR، QA کامل، مرحله‌بندی با OIDC، بازبینی artifact و تأیید نهایی با 2FA.
...
برای انتشار پایدار بعدی، نسخه رو به `0.2.0` تغییر بده، تغییر رو روی `main` merge کن و workflow با نام `Stage npm package` رو دستی اجرا کن:

- مقدار `version` برابر `0.2.0`
...
بعد از تأیید npm، workflow با نام `Release` رو با مقدار `v0.2.0` اجرا کن. Draft Release باید شامل archive قفل‌شده، نمونه PDF انگلیسی و فارسی در هر سه نسخه و فایل هش‌ها باشه. فقط بعد از بازبینی این فایل‌ها، Release رو منتشر کن.
